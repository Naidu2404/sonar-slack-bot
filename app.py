...
def handle_sonar(ack, respond, command, body):
    ...
    if sub == "help":
        respond(
            "*📊 SonarCloud Bot — Commands*\n\n"
            "*Setup (do once per repo):*\n"
            "```"
            "/sonar add-repo <project-key> <org-slug>\n"
            "   Register a repo globally. Get the project-key from sonarcloud.io.\n\n"
            "/sonar remove-repo <project-key>\n"
            "   Remove a repo and all its channel configs."
            "```\n\n"
            "*Per-channel config (run in the channel you want reports in):*\n"
            "```"
            "/sonar track <project-key>\n"
            "   Start tracking a repo in this channel.\n\n"
            "/sonar untrack <project-key>\n"
            "   Stop tracking a repo in this channel.\n\n"
            "/sonar add-files <project-key> <path> [path ...]\n"
            "   Add file path filters (substring match). Only matched files appear in reports.\n"
            "   Example: /sonar add-files my-repo contract_staffing src/utils/staffing\n\n"
            "/sonar remove-files <project-key> <path> [path ...]\n"
            "   Remove file path filters for this channel.\n\n"
            "/sonar schedule <project-key> <frequency> [HH:MM]\n"
            "   Set when reports are sent. Time is 24h UTC, defaults to 09:00.\n"
            "   Frequency options: daily | weekly (Mon) | biweekly (every 2nd Mon) | monthly (1st)\n"
            "   Examples:\n"
            "     /sonar schedule my-repo daily 08:30\n"
            "     /sonar schedule my-repo weekly 14:00\n"
            "     /sonar schedule my-repo monthly"
            "```\n\n"
            "*Reports:*\n"
            "```"
            "/sonar report\n"
            "   Run a report now. Shows a picker if multiple repos are tracked here.\n\n"
            "/sonar list\n"
            "   List all repos tracked in this channel with their schedules.\n\n"
            "/sonar status <project-key>\n"
            "   Show the full config for a repo in this channel."
            "```"
        )
